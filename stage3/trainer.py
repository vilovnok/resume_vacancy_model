...
class Trainer:
    ...
    def __init__(
        ...
        self.jf_train_dataloader = self.create_dataloader(train_dataset['jf'], shuffle=True)
        self.jd_train_dataloader = self.create_dataloader(train_dataset['jd'], shuffle=True)
        self.js_train_dataloader = self.create_dataloader(train_dataset['js'], shuffle=True)
        ...
        objs = [self.model,
                self.optimizer,
                self.jf_train_dataloader,
                self.jd_train_dataloader,
                self.js_train_dataloader,
                self.lr_scheduler]
    ...
    def create_lr_scheduler(self):
        ...
        steps_per_epoch = len(self.jf_train_dataloader)
    ...
    def train(self):
        ...
        self.logger.info(f"  Num examples = {len(self.train_dataset['jf'])}")
        ...
        self.logger.info(f"  Total optimization steps = {len(self.jf_train_dataloader) * self.args.num_epochs}")
    ...
    def train_epoch(self, epoch: int) -> Dict[str, float]:
        ...
        train_dataloader = zip(self.jf_train_dataloader, self.jd_train_dataloader, self.js_train_dataloader)

        progress_bar = tqdm(
            train_dataloader,
            total=min(
                len(self.jf_train_dataloader),
                len(self.jd_train_dataloader),
                len(self.js_train_dataloader)
            ),
            desc=f"Training epoch {epoch + 1}",
            disable=not self.accelerator.is_local_main_process
        )
        for jf_batch, jd_batch, js_batch in progress_bar:
            with self.accelerator.accumulate(self.model):

                jf_loss = self.training_step(jf_batch)
                jd_loss = self.training_step(jd_batch)
                js_loss = self.training_step(js_batch)
                
                total_loss = 0.5 * jf_loss['loss'] + 0.3 * jd_loss['loss'] + 0.2 * js_loss['loss']
                ...
                gathered_loss = self.accelerator.gather(total_loss).mean().item()
                epoch_loss += gathered_loss
                num_batches += 1
               
                progress_bar.set_postfix({
                    "loss": f"{gathered_loss:.4f}",
                    "jf": f"{jf_loss['loss'].detach().cpu():.4f}",
                    "jd": f"{jd_loss['loss'].detach().cpu():.4f}",
                    "js": f"{js_loss['loss'].detach().cpu():.4f}",
    ...
    def save_checkpoint(self, name: str):
        ...
        trainer_state = {
            'epoch': self.global_step // len(self.jf_train_dataloader),
