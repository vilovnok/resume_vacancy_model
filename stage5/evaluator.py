...
class Evaluator:
    ...
    def _encode_texts(self, texts: List[str], desc: str)-> torch.Tensor:
        ...
        with torch.no_grad():
            for i in tqdm(range(0, len(texts), self.batch_size), desc=f"Encoding {desc}"):
                ...
                emb = torch.tensor(emb['embeddings'], device=self.device)
    ...
    def compute_final_metrics(self, prefix: str="ft", device: str="cuda") -> None:
        ...
        if index_path.exists():
            ...
            dimension = doc_embeddings['embeddings'].shape[1]
            index = faiss.IndexFlatIP(dimension)
            index.add(doc_embeddings['embeddings'])
        ...
        D, I = index.search(query_embeddings['embeddings'], k_max)
