...
class ContrastiveTrainer:
    ...
    def train_epoch(self, epoch: int) -> Dict[str, float]:
        ...
        for batch in progress_bar:
            with self.accelerator.accumulate(self.student):
                ...
                loss_sim = self.loss_sim_step(batch)                                 
                loss = loss_sim['loss']
    ...
    def distill_pair(
        ...
        student_proj = self.projection(student_emb)
        student_proj = F.normalize(student_proj, dim=-1)
        teacher_emb = F.normalize(teacher_emb, dim=-1)
    
        loss_mse = F.mse_loss(student_proj, teacher_emb.detach())
        loss_cos = 1 - F.cosine_similarity(student_proj, teacher_emb.detach(), dim=-1).mean()
    
        return 0.5 * loss_mse + 0.5 * loss_cos


    def loss_sim_step(self, batch) -> Dict[str, torch.Tensor]:
        
        batch_student, batch_teacher = batch
        
        (anchor_s, pos_s, neg_s) = self.gen_embedding(
            model=self.student,
            batch=batch_student
        )
        
        with torch.no_grad():
            (anchor_t, pos_t, neg_t) = self.gen_embedding(
                model=self.teacher,
                batch=batch_teacher
            )
    
        loss_a = self.distill_pair(anchor_s, anchor_t)
        loss_p = self.distill_pair(pos_s, pos_t)
        loss_n = self.distill_pair(neg_s, neg_t)
    
        loss = (loss_a + loss_p + loss_n) / 3
    
        return {
            "loss": loss
        }

    def gen_embedding(self, model, batch) -> Dict[str, torch.Tensor]:        
        ...
        if batch['negative_input_ids'] is not None:
            ...
            neg_embeddings_flat = model(neg_ids, neg_mask)
